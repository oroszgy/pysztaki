...
class SztakiQueryParser:
    def __init__(self, from_lang = Languages.English, to_lang=Languages.Hungarian, base_url = BASE_URL):
    ...
    def build_query(self,word):
        return "{}?{}={}&{}={}&{}={}&in_pysztaki=1".format(
            self._base_url, FROM_KW, self._from, TO_KW, self._to, WORD_KW, quote(word))
    ...
    def parse_article(self, div):
        ...
        return [e.find("a").text.strip() for e in 
                act.findAll("div", {"class":"translation"}) if not e.find("span", {"class":"type_text"})]
